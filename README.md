...
# Versão recomendada do python 3.12.5
...
````bash
    ...
    Instalação: O download foi feito do arquivo do CUDA Toolkit e a instalação foi realizada usando a opção "Expressa".
````
#### Passo 3: Instalação da Biblioteca cuDNN

A cuDNN é uma biblioteca que otimiza operações de redes neurais profundas na GPU.
````bash
    Ação: Fazer o download da versão da cuDNN compatível com o CUDA 11.8 no site da NVIDIA cuDNN.
